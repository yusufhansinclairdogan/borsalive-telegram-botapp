from fastapi import (
    ...
    Response, Header, HTTPException, Query
)
...
def _decode_trade_payload(u8: bytes) -> dict:
    ...
    while i < L:
        ...
        if wt == 0:  # varint
            ...
            if field_no == 4: out["qty"] = v
            elif field_no == 6: out["ts"] = v
            ...
            try:
                ...
                val = ""
            if   field_no == 1: out["symbol"] = val
            elif field_no == 2: out["trade_id"] = val
            elif field_no == 5: out["side"] = val
            elif field_no == 7: out["buyer"] = val
            elif field_no == 8: out["seller"] = val
        elif wt == 5:  # 32-bit (price)
            v, i = _read_f32_le(u8, i)
            if field_no == 3: out["price"] = v
    ...
    try:
        while websocket.application_state == WebSocketState.CONNECTED:
            try:
                ...
                log.error("[%s]: depth runner error; retrying shortly", cid, exc_info=True)
...
async def ws_trade(ws: WebSocket, symbol: str):
    ...
    client = MatrixTradeClient(
        symbol=sym,
        connect_template_b64=(getattr(settings, "TRADE_CONNECT_TEMPLATE_B64", "") or settings.CONNECT_TEMPLATE_B64),
    )
    ...
    def _read_len(buf: bytes, i: int):
        ln, i = _read_varint(buf, i); j = i + ln; return buf[i:j], j
    ...
    def _mini_decode(u8: bytes) -> dict:
        ...
        while i < L:
            ...
            if wt == 0:
                ...
                if   f == 1: out["symbol"] = s
                elif f == 2: out["trade_id"] = s
                elif f == 5: out["side"] = s
                elif f == 7: out["buyer"] = s
                elif f == 8: out["seller"] = s
            elif wt == 5:
                v, i = _read_f32(u8, i)
                if f == 3: out["price"] = v
            elif wt == 1:
    ...
    def _normalize(t: dict) -> dict:
        ...
        if not t.get("symbol"): t["symbol"] = sym
        # side tek harf (a/b) ve küçük
        if t.get("side"): t["side"] = str(t["side"]).lower()[:1]
        ...
        t["buyer"]  = t.get("buyer")  or t.get("buyer_code")  or t.get("buyerTag")  or t.get("b") or ""
        t["seller"] = t.get("seller") or t.get("seller_code") or t.get("sellerTag") or t.get("s") or ""
        # sayı tipleri
        try: t["price"] = float(t.get("price") or 0.0)
        except: t["price"] = 0.0
        try: t["qty"] = int(t.get("qty") or 0)
        except: t["qty"] = 0
        return t
        ...
        while ws.application_state == WebSocketState.CONNECTED:
            try:
                async for item in client.connect_and_stream():
                    # 1) trade_proxy dict döndürüyorsa direkt kullan
                    ...
                    # 2) yine de bytes gelirse minidecoder
                    if isinstance(item, (bytes, bytearray)):
                        try:
                            ...
                            pass
                        # çok mecbur kalınırsa base64 fallback
                        try:
                            import base64
...
async def snapshot_depth(
    ...
    sym = symbol.upper()
    levels = await hub.get_last(sym, timeout=1.0) or []
    # trades entegrasyonunu sonra ekleyeceğiz
    png = render_depth_png(levels, sym, size=size, scale=scale)
...
async def logo(symbol: str):
    ...
    if r.status_code == 200 and r.content:
        ...
        if   "svg"  in ctype: mt = "image/svg+xml"
        elif "png"  in ctype: mt = "image/png"
        elif "jpeg" in ctype or "jpg" in ctype: mt = "image/jpeg"
        elif "gif"  in ctype: mt = "image/gif"
...
async def ws_market(ws: WebSocket, symbol: str):
    ...
    client = MatrixMarketClient(symbol=sym, connect_template_b64=getattr(settings, "MARKET_CONNECT_TEMPLATE_B64", "") or settings.CONNECT_TEMPLATE_B64)
    try:
        async for payload in client.connect_and_stream():
            try:
                b64 = base64.b64encode(payload).decode("ascii")
                await ws.send_text(b64)
            except (WebSocketDisconnect, RuntimeError):  # RuntimeError: close after close
                break
    except WebSocketDisconnect:
        pass
    except Exception:
